...
class ReportModel extends ChangeNotifier {
  ...
  Future<void> fetchComparisonData(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        double totalReceived1 =
            filteredOrders1.fold(0, (sum, order) => sum + order.receivedMoney);
        double totalReceived2 =
            filteredOrders2.fold(0, (sum, order) => sum + order.receivedMoney);
        double totalRevenue1 =
            filteredOrders1.fold(0, (sum, order) => sum + order.totalPrice);
        double totalRevenue2 =
            filteredOrders2.fold(0, (sum, order) => sum + order.totalPrice);
        ...
        int productNumberSold1 = filteredOrders1.fold<int>(0, (sum, order) {
          return sum + order.orderDetails.fold<int>(0, (detailSum, detail) {
        ...
        productNumberSoldPercent = calculatePercentageChangeInt(productNumberSold1, productNumberSold2);


        listOrders = {
          for (var order in [...filteredOrders1, ...filteredOrders2]) order.id: order
        }.values.toList();

        getPaymentMethodData(listOrders);

        List<MapEntry<String, int>> rawBestSellingSizes = getTop5BestSellingSizes([...filteredOrders1, ...filteredOrders2], listProducts);
  ...
  List<MapEntry<String, int>> getTop5BestSellingSizes(List<Order> orders, List<Product> listProducts) {
    ...
    for (Order order in orders) {
      for (OrderDetail detail in order.orderDetails) {
        ...
        sizeSales.update(key, (existingQuantity) => existingQuantity + quantity, ifAbsent: () => quantity);
      }
    }
